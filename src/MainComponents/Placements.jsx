...
const JOB_TYPES = ["Online", "Offline"];
      ...
      <section className="relative mt-24 flex min-h-[50vh] items-center justify-center overflow-hidden bg-[radial-gradient(120%_160%_at_100%_0%,#1c5872,#12171B_70%)] px-6 py-20">
        ...
        <div className="relative z-10 mx-auto max-w-5xl text-center">
          ...
          <p className="mx-auto mb-8 max-w-2xl text-lg text-slate-300">
            A refined space for fresh opportunities, curated roles, and career-ready pathways.
          </p>
          ...
          <aside className="w-full shrink-0 h-fit top-10 sticky md:top-28 md:h-fit md:w-80">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:shadow-[0_8px_30px_rgba(15,23,42,0.06)]">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-sm font-bold uppercase tracking-[0.15em] text-slate-500">Filters</h3>
                  <p className="mt-2 text-sm text-slate-500">Narrow jobs by experience, state, and job type.</p>
                </div>
                {hasActiveFilters && (
                  <button onClick={clearFilters} className="text-xs font-semibold text-[#E8791E] hover:text-[#F2A93C] transition-colors">
                    Clear
                  </button>
                )}
              </div>
              ...
              <div className="md:space-y-5 flex md:flex-col md:gap-0 gap-2">
                ...
                {/* Location Filter */}
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Location</label>
                  <select
                ...
                {/* Job Type Filter */}
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Job Type</label>
                  <select
              ...
              <div className="mt-4 rounded-xl bg-[#FFF7ED] px-3 py-2.5 text-xs text-slate-600">
                <span className="font-semibold text-[#E8791E]">{filteredJobs.length}</span> job{filteredJobs.length !== 1 ? "s" : ""} found
              </div>
            </div>
          </aside>

          {/* Scrollable Job Cards Container */}
          <div className="flex-1 overflow-y-auto pr-0 sm:pr-2 lg:pr-4" style={{ maxHeight: "calc(100vh - 14rem)" }}>
            {loading ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-8 py-16 text-center">
                ...
                <p className="mx-auto mt-2 max-w-md text-sm text-slate-500">
                  Try adjusting your filter selections to see more opportunities.
                </p>
                {hasActiveFilters && (
                  <button onClick={clearFilters} className="mt-4 rounded-full bg-[#E8791E] px-6 py-2 text-sm font-semibold text-white transition hover:bg-[#F2A93C]">
                    Clear filters
                  </button>
                )}
              </div>
