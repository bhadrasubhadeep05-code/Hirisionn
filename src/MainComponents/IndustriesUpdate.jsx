...
const IndustriesUpdate = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const res = await getLetastIndustryBlog();
        setBlog(res);
      } catch (err) {
    ...
    const fetchVideo = async () => {
      try {
        const res = await getLetastIndustryVideo();
        setVideo(res);
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-[#4A5568] max-w-2xl mx-auto">
            Stay ahead with the latest trends in staffing, recruitment, and career development
          </p>
        </motion.div>
        ...
        <div className="mb-16 sm:mb-20">
          ...
          >
            ...
            <motion.button
              onClick={() => navigate(`/industries-update-blog`)}
        ...
        <div>
          ...
          >
            ...
            <motion.button
              onClick={() => navigate(`/industries-update-video`)}
              className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-[#22D3EE]/20 to-[#818CF8]/20 border border-[#22D3EE]/40 rounded-full text-[#22D3EE] font-semibold text-sm sm:text-base hover:border-[#22D3EE]/70 transition-all duration-300"
              whileHover={{ scale: 1.05, x: 4 }}
            >
              See More
...
export default IndustriesUpdate
