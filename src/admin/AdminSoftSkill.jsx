...
import { getSoftSkill, updateSoftSkill  } from "../services/admin.api";
...
const AdminSoftSkill = () => {
  ...
  const fetchApplicants = async () => {
    try {
      setLoading(true);
      const res = await getSoftSkill();
      console.log("Soft Skill applicants:", res.users)
  ...
  // Get all soft skill applications with status
  ...
  const appliedCount = getAllApplications().filter(app => app.skill.status === "Applied").length;
  const fulfilledCount = getAllApplications().filter(app => app.skill.status === "Fulfilled").length;

  // Filter applications based on selected status
  const filteredApplications = getAllApplications().filter(app => {
    if (filter === "all") return true;
    if (filter === "applied") return app.skill.status === "Applied";
    if (filter === "fulfilled") return app.skill.status === "Fulfilled";
    return true;
  });

  const handleClick = async (id) => {
    try {
      const res = await updateSoftSkill({
        userId: id
      });
          ...
          >
            <h1 className="text-4xl md:text-5xl font-extrabold mb-3">
              <span className="bg-clip-text text-transparent bg-gradient-to-b from-[#0F172A] via-[#475569] to-[#0F172A]">
                Soft Skill Applicants
              </span>
            </h1>
            <p className="text-slate-500 font-medium text-lg">
              Manage and track soft skill program applications
            </p>
          </motion.div>
                    ...
                    {filteredApplications.map((app, idx) => (
                      <tr key={idx} className={`border-b border-gray-100 ${idx % 2 === 0 ? "bg-white" : "bg-[#F8FAFC]"}`}>
                        ...
                        <td className="px-6 py-4 text-sm">
                          {new Date(app.skill.appliedAt).toLocaleDateString()}
                        </td>
                        <td className="px-6 py-4">
                          {app.skill.status === "Fulfilled" ? (
