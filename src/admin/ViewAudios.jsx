...
import { getAllInterviewAudio, getAllIndustryAudio, getAllOtherAudioCon } from '../services/audio';
...
const ViewAudios = () => {
  ...
  const loadAudios = async () => {
    ...
    try {
      ...
      if (activeTab === 'interview') {
        response = await getAllInterviewAudio();
      } else if (activeTab === 'industry') {
        response = await getAllIndustryAudio();
      } else {
        response = await getAllOtherAudioCon();
      }
      if (response.data) {
  ...
  const handleDeleteAudio = async (audioId) => {
    if (window.confirm('Are you sure you want to delete this audio? This action cannot be undone.')) {
      setDeletingId(audioId);
      try {
        await deleteAudio(audioId);
        setAudios(prev => prev.filter(audio => audio._id !== audioId));
      } catch (err) {
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-slate-500 font-medium text-lg">
              View and manage all audio podcast content.
            </p>
          </motion.div>
            ...
            <AnimatePresence mode="popLayout">
              {loading ? (
                <div className="col-span-full text-center py-20">
                  <p className="text-slate-500 font-medium">Loading audios...</p>
                </div>
              ) : audios.length === 0 ? (
                <div className="col-span-full text-center py-20">
                  <p className="text-slate-500 font-medium">No audios found in this category</p>
                </div>
              ) : (
